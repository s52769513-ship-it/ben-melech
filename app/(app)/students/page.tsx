    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <Suspense fallback={<p className="text-gray-500 mt-1">טוען…</p>}>
          <StudentsSummary searchParams={searchParams} />
        </Suspense>
      </div>
...
// Both boundaries below need the same list — cache() keeps it to one pass per
// request instead of resolving it twice.
const resolveStudents = cache(async (searchParams: Promise<Filters>) => {
  // Request data first: the Airtable reads below must never run during the
  // prerender, only against a real request.
  ...
  const effectiveFilters =
    coordinatorId && coordinatorId !== "ADMIN"
      ...
      : filters;
  return { filters, students: await getStudents(effectiveFilters) };
});

async function StudentsSummary({ searchParams }: { searchParams: Promise<Filters> }) {
  const { students } = await resolveStudents(searchParams);
  return (
