...
import { Loader2, Search, ChevronDown, ChevronUp, RefreshCw, UserPlus, Store, Users } from "lucide-react";
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        <button onClick={() => setView("bochurim")}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input type="text" placeholder="חיפוש בחור..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          {loading ? (
            ...
            <div className="py-10 text-center">
              ...
              <button onClick={load} className="text-sm text-blue-600 hover:underline">נסה שוב</button>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              <div className="grid grid-cols-[1fr_140px_120px_40px] gap-2 px-6 py-2 bg-gray-50 text-xs text-gray-400 font-medium">
                <span>בחור</span><span>קטגוריה</span><span className="text-center">יתרה</span><span />
              </div>
              {filtered.length === 0 ? (
                ...
                filtered.map((b) => {
                  ...
                  return (
                    <div key={b.ClientId}>
                      <button onClick={() => setExpanded(isOpen ? null : b.ClientId ?? null)}
                        className="w-full grid grid-cols-[1fr_140px_120px_40px] gap-2 items-center px-6 py-3 text-right hover:bg-gray-50">
                        <div>
                          <span className="text-sm font-medium text-gray-800">{b.FirstName} {b.FamilyName}</span>
                          <span className="text-xs text-gray-400 mr-2">#{b.ClientId}</span>
                        </div>
                        <span className="text-xs text-gray-500">{b.Groupe || "—"}</span>
                        <span className="text-sm text-center font-medium text-green-600">₪{parseAmount(b.Ytra).toLocaleString()}</span>
                      ...
                      {isOpen && b.ClientId && (
                        <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-100">
                          <BochurPanel
                            ...
                            onBalanceChange={(bal) =>
                              setList((prev) => prev.map((x) => x.ClientId === b.ClientId ? { ...x, Ytra: String(bal) } : x))
                            }
                          />
                        </div>
                      )}
...
function AddBochur({ onDone }: { onDone: () => void }) {
  const [form, setForm] = useState({ FamilyName: "", FirstName: "", Zeout: "", Phone1: "", Address: "", Groupe: "" });
  ...
  async function save() {
    if (!form.FamilyName.trim()) { setErr("יש להזין שם בחור"); return; }
    setSaving(true); setErr(null);
    const res = await callCard("SaveClientCard", form);
    setSaving(false);
    if (isOk(res)) { onDone(); }
    else { setErr(res.Message || "ההוספה נכשלה"); }
  }

  const fields: { key: keyof typeof form; label: string }[] = [
    { key: "FamilyName", label: "שם הבחור (משפחה)" }, { key: "FirstName", label: "שם פרטי" },
    { key: "Zeout", label: "תעודת זהות" }, { key: "Phone1", label: "טלפון" },
    ...
    <div className="px-6 py-4 bg-blue-50/50 border-b border-blue-100 space-y-3">
      ...
      <div className="flex gap-2">
        <button onClick={save} disabled={saving}
          ...
          {saving ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} />} הוסף
        </button>
        <button onClick={onDone} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700">ביטול</button>
      </div>
    </div>
  );
}
