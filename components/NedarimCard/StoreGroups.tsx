...
export default function StoreGroups() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const [g, s] = await Promise.all([callCard("GetLimitedStoresList"), callCard("GetStoresList")]);
  ...
  async function remove(group: Group) {
    ...
    const res = await callCard("SaveLimitedStores", { ListId: group.ID, ListName: group.ListName, Delete: 1 });
    setBusy(null);
    if (isOk(res)) { flash(true, "הקבוצה נמחקה"); load(); }
    else { flash(false, res.Message || "המחיקה נכשלה"); }
  }

  if (loading) {
    return <div className="flex items-center justify-center gap-2 py-12 text-gray-400"><Loader2 size={18} className="animate-spin" /> טוען...</div>;
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      {notice && (
        <div className={`text-sm px-6 py-2 ${notice.ok ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
          {notice.text}
        </div>
      )}
        ...
        <div className="divide-y divide-gray-100">
          {groups.map((g) => (
            <div key={g.ID} className="px-6 py-3 flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-medium text-gray-800">{g.ListName} <span className="text-xs text-gray-400">#{g.ID}</span></p>
                <p className="text-xs text-gray-500 mt-0.5">{(g.Stores ?? []).map((s) => s.StoreName).join(", ") || "אין חנויות"}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => setEditing(g)} className="text-xs text-blue-600 hover:underline px-2 py-1">עריכה</button>
                <button onClick={() => remove(g)} disabled={busy === `del-${g.ID}`}
                  className="text-red-500 hover:text-red-700 p-1 disabled:opacity-50">
                  {busy === `del-${g.ID}` ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}
                </button>
              </div>
            </div>
          ))}
...
function GroupForm({ group, stores, onClose, onSaved, flash }: {
  group: Group | null;
  stores: Store[];
  onClose: () => void;
  onSaved: () => void;
  ...
  const [selected, setSelected] = useState<string[]>((group?.Stores ?? []).map((s) => s.StoreId ?? "").filter(Boolean));
  const [saving, setSaving] = useState(false);

  function toggle(id: string) { setSelected((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]); }

  async function save() {
    if (!name.trim()) { flash(false, "יש להזין שם קבוצה"); return; }
    if (selected.length === 0) { flash(false, "יש לבחור לפחות חנות אחת"); return; }
    setSaving(true);
    const res = await callCard("SaveLimitedStores", { ListId: group?.ID, ListName: name, StoresId: selected.join(",") });
    setSaving(false);
    if (isOk(res)) { flash(true, group ? "הקבוצה עודכנה" : "הקבוצה נוספה"); onSaved(); }
    else { flash(false, res.Message || "השמירה נכשלה"); }
  }

  return (
    <div className="px-6 py-4 bg-blue-50/50 border-b border-blue-100 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-[#1e3a5f]">{group ? `עריכת קבוצה: ${group.ListName}` : "קבוצה חדשה"}</p>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-700"><X size={16} /></button>
      </div>
      <label className="block text-sm">
        <span className="text-gray-500 text-xs">שם הקבוצה {group && "(לא ניתן לשינוי)"}</span>
        <input type="text" value={name} disabled={!!group} onChange={(e) => setName(e.target.value)}
