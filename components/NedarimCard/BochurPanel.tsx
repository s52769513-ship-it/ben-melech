...
export default function BochurPanel({
  ...
  async function addTlush() {
    if (!amount || parseFloat(amount) <= 0) { flash(false, "יש להזין סכום תקין"); return; }
    if (!password) { flash(false, "יש להזין סיסמת אישור להטענה"); return; }
    setBusy("add");
    const res = await callCard("AddTlush", {
    ...
    if (isOk(res)) {
      ...
      setAmount(""); setExpiration(""); setChargeComments("");
      await load();
    } else {
  ...
  async function unloadTlush(tlushId?: string) {
    if (!tlushId) return;
    if (!password) { flash(false, "יש להזין סיסמת אישור לפריקה"); return; }
    ...
    setBusy(null);
    if (isOk(res)) { flash(true, "הטעינה נפרקה בהצלחה"); await load(); }
    else { flash(false, res.Message || "הפריקה נכשלה"); }
  }

  async function addMagnetic() {
    if (!newMagnetic) { flash(false, "יש להזין מספר כרטיס מגנטי"); return; }
    setBusy("magnetic-add");
    const res = await callCard("SetClientMagneticCard", { ClientId: clientId, MagneticCard: newMagnetic });
    setBusy(null);
    if (isOk(res)) { flash(true, "הכרטיס שויך בהצלחה"); setNewMagnetic(""); await load(); }
    else { flash(false, res.Message || "שיוך הכרטיס נכשל"); }
  }
  ...
  async function removeMagnetic(card: MagneticCard) {
    ...
    const res = await callCard("SetClientMagneticCard", {
      ClientId: clientId, CardId: card.CardId, MagneticCard: card.MagneticCard, Remove: 1,
    });
    setBusy(null);
    if (isOk(res)) { flash(true, "הכרטיס נותק בהצלחה"); await load(); }
    else { flash(false, res.Message || "מחיקת הכרטיס נכשלה"); }
  }

  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-10 text-gray-400">
        <Loader2 size={18} className="animate-spin" /> טוען נתוני בחור...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="py-8 text-center">
        ...
        <button onClick={load} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs border border-gray-300 rounded-lg hover:bg-gray-50">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={load} className="p-2 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-100" title="רענן">
            <RefreshCw size={15} />
          </button>
        </div>
      </div>
      ...
      {notice && (
        <div className={`text-sm rounded-lg px-3 py-2 ${notice.ok ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
          {notice.text}
        </div>
      )}
        ...
        {TABS.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-sm whitespace-nowrap border-b-2 -mb-px transition-colors ${
              tab === key ? "border-[#1e3a5f] text-[#1e3a5f] font-medium" : "border-transparent text-gray-500 hover:text-gray-700"
            }`}
          >
            <Icon size={14} />{label}
          </button>
        ...
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <input type="text" placeholder="תפוגה dd/mm/yyyy" value={expiration} onChange={(e) => setExpiration(e.target.value)}
                className="border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400" />
              <select value={limitedId} onChange={(e) => setLimitedId(e.target.value)}
                className="border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400">
                <option value="">ללא הגבלת חנויות</option>
                {groups.map((g) => <option key={g.ID} value={g.ID}>{g.ListName}</option>)}
              </select>
              <input type="text" placeholder="הערה" value={chargeComments} onChange={(e) => setChargeComments(e.target.value)}
                className="border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400" />
            </div>
            <div className="flex items-center gap-2">
              <input type="password" placeholder="סיסמת אישור" value={password} onChange={(e) => setPassword(e.target.value)}
                className="border border-amber-300 rounded px-2 py-1.5 text-sm w-40 focus:outline-none focus:ring-1 focus:ring-amber-400" />
              <button onClick={addTlush} disabled={busy === "add"}
                ...
                {busy === "add" ? <Loader2 size={14} className="animate-spin" /> : <CreditCard size={14} />} הטען
              </button>
            </div>
          </div>
          <Table
            ...
            rows={(data?.Tlushim ?? []).map((t) => [
              t.TlushId ?? "—", t.Date ?? "—",
              t.Amount ? `₪${t.Amount}` : "—", t.FreeAmount ?? "—",
              t.Expiration ?? "—", t.LimitedStores ?? "—",
              <button key="x" onClick={() => unloadTlush(t.TlushId)} disabled={busy === `unload-${t.TlushId}`}
      ...
      {tab === "history" && (
        <Table
          ...
          rows={(data?.History ?? []).map((h) => [h.Date ?? "—", h.StoreName ?? "—", h.Amount ? `₪${h.Amount}` : "—", h.Comments ?? "—"])}
      ...
      {tab === "cards" && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <input type="text" placeholder="מספר כרטיס / פס מגנטי" value={newMagnetic} onChange={(e) => setNewMagnetic(e.target.value)}
              className="border border-gray-300 rounded px-2 py-1.5 text-sm flex-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
            <button onClick={addMagnetic} disabled={busy === "magnetic-add"}
              ...
              {busy === "magnetic-add" ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />} שייך כרטיס
            </button>
          </div>
          <Table
            head={["מספר", "מגנטי", "שויך", "נותק", ""]}
            rows={(data?.Cards ?? []).map((c) => [
              c.CardNumber ?? "—", c.MagneticCard ?? "—", c.AddedDate ?? "—", c.RemovedDate || "—",
              c.RemovedDate ? (
                <span key="x" className="text-xs text-gray-400">מנותק</span>
              ) : (
                <button key="x" onClick={() => removeMagnetic(c)} disabled={busy === `magnetic-del-${c.CardId}`}
                  className="inline-flex items-center gap-1 text-xs text-red-500 hover:text-red-700 disabled:opacity-50">
                  {busy === `magnetic-del-${c.CardId}` ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} נתק
                </button>
              ),
            ])}
      ...
      {tab === "siruvim" && (
        <Table
          ...
          rows={(data?.Siruvim ?? []).map((s) => [s.Date ?? "—", s.StoreName ?? "—", s.Amount ? `₪${s.Amount}` : "—", s.Error ?? "—"])}
          empty="אין סירובים"
        />
      )}
      ...
      {tab === "auto" && (
        <Table
          ...
          rows={(data?.AutoCharge ?? []).map((a) => [
            ...
            a.Amount ? `₪${a.Amount}` : "—",
            a.NextCharge ?? "—", a.HebNextCharge ?? "—", a.Comments ?? "—",
          ])}
...
function Table({ head, rows, empty }: { head: string[]; rows: React.ReactNode[][]; empty: string }) {
  if (rows.length === 0) return <p className="text-gray-400 text-sm text-center py-6">{empty}</p>;
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>{head.map((h, i) => <th key={i} className="text-right px-3 py-2 font-medium text-gray-500 whitespace-nowrap">{h}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-50">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-gray-50">
              {row.map((cell, j) => <td key={j} className="px-3 py-2 text-gray-700 whitespace-nowrap">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
...
function DetailsForm({ clientId, data, onSaved, flash }: {
  clientId: string | number;
  data: CardData | null;
  onSaved: () => void;
  ...
  const [form, setForm] = useState({
    FirstName: data?.FirstName ?? "", FamilyName: data?.FamilyName ?? "",
    Zeout: data?.Zeout ?? "", Address: data?.Address ?? "",
    Phone1: data?.Phone1 ?? "", Phone2: data?.Phone2 ?? "",
    Email: data?.Email ?? "", Groupe: data?.Groupe ?? "", Comments: data?.Comments ?? "",
  });
  ...
  async function save() {
    ...
    setSaving(false);
    if (isOk(res)) { flash(true, "פרטי הבחור נשמרו"); onSaved(); }
    else { flash(false, res.Message || "שמירת הפרטים נכשלה"); }
  }

  const fields: { key: keyof typeof form; label: string }[] = [
    { key: "FamilyName", label: "שם הבחור (משפחה)" }, { key: "FirstName", label: "שם פרטי" },
    { key: "Zeout", label: "תעודת זהות" }, { key: "Phone1", label: "טלפון 1" },
    { key: "Phone2", label: "טלפון 2" }, { key: "Email", label: "מייל" },
    { key: "Address", label: "כתובת" }, { key: "Groupe", label: "קטגוריה" },
  ...
  return (
    <div className="space-y-3">
      ...
      <button onClick={save} disabled={saving}
        ...
        {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} שמור פרטים
      </button>
    </div>
  );
}
